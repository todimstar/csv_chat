...
6.  **调整文件上传大小限制** (适用于大型CSV文件):
    *   Streamlit 默认的文件上传大小限制为 200MB，如果您需要分析更大的 CSV 文件，可以通过以下方式调整限制:
        ...
        *   **方法二**: 创建配置文件
            在项目根目录创建 `.streamlit/config.toml` 文件，添加以下内容:
            ```toml
            [server]
            maxUploadSize = 1028
            ```
        ...
        *   **方法三**: 在代码中设置环境变量 (不推荐，但可作为临时解决方案)
            ...
            ```
    *   **注意**: 即使提高了上传限制，处理超大文件仍可能导致内存问题，因为整个文件会被加载到内存中，这取决于运行设备。且当文件大于150MB时网页便开始有明显加载痕迹，这取决于py运行速度~~和代码质量(悲)~~
    *   **受影响的前端**: ![配置.streamlit里config.toml为500之后](./docs/imges/配置.streamlit里config.toml为500之后.png)
